"""
Build a neofetch-style info card SVG: colored key/value rows for education,
current project, tech stack, and highlights. Lines fade/slide in on a short
stagger so it feels like the panel is printing alongside the portrait.
...
ROWS = [
    ("host",),
    ("kv", "Now", "Churn Prediction ML System"),
    ("kv", "Edu", "BTech Student, Bengaluru, India"),
    ("kv", "Focus", "ML Systems → Industry Grade"),
    ("gap",),
    ("sec", "Languages"),
    ("kv", "Core", "Python, Java, SQL"),
    ("gap",),
    ("sec", "AI / ML Stack"),
    ("kv", "Frameworks", "PyTorch, Scikit-learn"),
    ("kv", "Data", "Pandas, NumPy"),
    ("kv", "Domains", "ML, DL, NLP, RL"),
    ("gap",),
    ("sec", "Tools"),
    ("kv", "DevOps", "Git, GitHub, Docker, Linux"),
    ("kv", "Editor", "VS Code"),
    ("gap",),
    ("sec", "Highlights"),
    ("bul", "Building industry-grade ML pipelines"),
    ("bul", "Strong fundamentals in ML & DL"),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">venkat@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">venkat</tspan><tspan fill="{MUTED}">@</tspan>'
